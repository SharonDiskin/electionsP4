...
vector <Citizen*> loadAllCitizens(const string fileName)
{
	ifstream inFile(fileName, ios::binary | ios::in);
	int numOfCitizens;
	inFile >> numOfCitizens;
	vector <Citizen*> citizensFromFile(numOfCitizens);
	...
	for (int i = 0; i < numOfCitizens; i++)
	{
		...
		if (strcmp(type, typeid(Citizen).name()+10) == 0)
			citizensFromFile[i] = new Citizen(inFile);
		else if (strcmp(type, typeid(Contestant).name()+10) == 0)
			citizensFromFile[i] = (Citizen*)new Contestant(inFile);
		else if (strcmp(type, typeid(Soldier).name() + 10) == 0)
			citizensFromFile[i] = (Citizen*) new Soldier(inFile);
	}
	inFile.close();
	return citizensFromFile;
}
...
void saveCitizens(vector <Citizen*> allCitizens, int size, const char* fileName)
{
	...
	{
		outFile << typeid(*allCitizens[i]).name()+10 << " " << *allCitizens[i] << endl;
	}
...
void createCitizen(ElectionRound& elections, int opt) throw (const char*, const char*)
...
//This function create the menu for this election round.
{
	...
	ifstream inFile("citizens4.txt"); //We open the file in order to see if there is something in the file

	if(inFile) //If there is something in the file we set the citizen note the loaded file
	{
	  election.setCitizenNote(loadAllCitizens("citizens4.txt"));
	}
	...
	do {
		...
		case 2://add new citizen.
			if (isElectionNotOver)
			{
				...
				catch (const char* msg) //If the id in invalid or birth year is invalid we throw an exception
				{
					cout << msg << endl;
				}
			}
		...
		case 10://ending the program.
			saveCitizens(election.getCitizenNote(), election.getCitizenNote().size(), "citizens4.txt"); //Write the citizens to the file
			cout << "Bye bye!\n";
			fContinue = false;
		}
