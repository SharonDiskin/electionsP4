...
const ElectionRound& ElectionRound:: operator+=(Citizen& citizen)
{
	...
	if (citizenNote.size() == citizenNote.capacity()) //If the size of the vector equals to the vector capacity - we increase capcity by 2
	...
	else // he is a contestant - we add him as a contestant
	{
		...
		partyList[index]->addContestant(*tempCont);
	}
